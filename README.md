...
- **การเข้าถึงที่ครอบคลุม (Accessibility & WCAG Compliance)** — ออกแบบตามมาตรฐาน WAI-ARIA ครบถ้วน ทั้ง `role="search"`, `role="dialog"`, `aria-modal`, `aria-label`, การนำทางและเปิดการ์ดเพลงด้วยแป้นพิมพ์ (Song Card Keyboard Navigation & Enter/Space Support), การควบคุมและดักจับลำดับโฟกัสในหน้าต่างเนื้อเพลง (Modal Dialog Keyboard Focus Trap & Loop Containment), และการแจ้งเตือน Toast ด้วย `role="status"` / `aria-live="polite"` พร้อม Focus Indicator (`:focus-visible`) ชัดเจนสำหรับการควบคุมผ่านคีย์บอร์ด, การลดการเคลื่อนไหวตามสเปกระบบ (`prefers-reduced-motion`), การคัดลอกที่ไม่กระตุกบน iOS, และ Mobile Viewport ที่ซูมขยายได้อย่างอิสระ
...
- **PWA & Offline First** — ติดตั้งลงบนหน้าจอสมาร์ตโฟนได้เหมือนแอป Native ใช้งานแบบ Offline ได้ผ่าน Service Worker Resilient Caching (`Promise.allSettled`, `thungara-v14`) และ Navigation Fallback พร้อม Web App Manifest มาตรฐานสากล
- **High Performance Web Worker** — แยกการคำนวณการค้นหาทั้งหมดออกจาก Main Thread พร้อม Pre-normalization เพิ่มความเร็วในการประมวลผลคำศัพท์และคำพ้อง ทำให้ UI ไม่กระตุก ด้วยความเร็วในการค้นหาเฉลี่ยต่ำกว่า 20ms

## Tech Stack

- **Frontend**: HTML5, CSS3, Modern JavaScript (Pure Vanilla — Zero External Frameworks)
- **Web Standards & A11y**: WAI-ARIA 1.2, WCAG 2.1 Focus-Visible & Reduced-Motion, Modal Focus Trap & Keyboard Loop, Screen-Reader Toast Live Regions, SafeStorage Wrapper, Path-Agnostic Asset Fallbacks
...
- **Offline & Cache**: Service Worker + Cache Storage API with Resilient Promise.allSettled Offline Caching (`thungara-v14`)
...
tests/
	test_search.py    — ชุดทดสอบ Core Search Engine (15 Categories, 49 Assertions)
	test_full_system.py — ชุดตรวจสอบและตรวจสอบความปลอดภัยทั้งระบบ (Comprehensive Audit Suite: 49 Checks)
	test_runner.html  — หน้าทดสอบบนเบราว์เซอร์พร้อม UI แสดงผลและจับเวลา Latency (15 Categories, 49 Tests)
