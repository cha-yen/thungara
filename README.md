...
- **การเข้าถึงที่ครอบคลุม (Accessibility & WCAG Compliance)** — ออกแบบตามมาตรฐาน WAI-ARIA ครบถ้วน ทั้ง `role="search"`, `role="dialog"`, `aria-modal`, `aria-label`, การจัดการลำดับโฟกัสในหน้าต่างเนื้อเพลง (Modal Focus Order), และการแจ้งเตือน Toast ด้วย `role="status"` / `aria-live="polite"` พร้อม Focus Indicator (`:focus-visible`) ชัดเจนสำหรับการควบคุมผ่านคีย์บอร์ด, การลดการเคลื่อนไหวตามสเปกระบบ (`prefers-reduced-motion`), การคัดลอกที่ไม่กระตุกบน iOS, และ Mobile Viewport ที่ซูมขยายได้อย่างอิสระ
...
tests/
	test_search.py    — ชุดทดสอบ Core Search Engine (13 Categories, 43 Assertions)
	test_full_system.py — ชุดตรวจสอบและตรวจสอบความปลอดภัยทั้งระบบ (Comprehensive Audit Suite: 41 Checks)
	test_runner.html  — หน้าทดสอบบนเบราว์เซอร์พร้อม UI แสดงผลและจับเวลา Latency (13 Categories, 43 Tests)
