...
- **การเข้าถึงที่ครอบคลุม (Accessibility & WCAG Compliance)** — ออกแบบตามมาตรฐาน WAI-ARIA ครบถ้วน ทั้ง `role="search"`, `role="dialog"`, `aria-modal`, `aria-label`, การนำทางและเปิดการ์ดเพลงด้วยแป้นพิมพ์ (Song Card Keyboard Navigation & Enter/Space Support), การจัดการลำดับโฟกัสในหน้าต่างเนื้อเพลง (Modal Focus Order), และการแจ้งเตือน Toast ด้วย `role="status"` / `aria-live="polite"` พร้อม Focus Indicator (`:focus-visible`) ชัดเจนสำหรับการควบคุมผ่านคีย์บอร์ด, การลดการเคลื่อนไหวตามสเปกระบบ (`prefers-reduced-motion`), การคัดลอกที่ไม่กระตุกบน iOS, และ Mobile Viewport ที่ซูมขยายได้อย่างอิสระ
...
- **PWA & Offline First** — ติดตั้งลงบนหน้าจอสมาร์ตโฟนได้เหมือนแอป Native ใช้งานแบบ Offline ได้ผ่าน Service Worker Resilient Caching (`Promise.allSettled`) และ Navigation Fallback พร้อม Web App Manifest มาตรฐานสากล
...
- **Web Standards & A11y**: WAI-ARIA 1.2, WCAG 2.1 Focus-Visible & Reduced-Motion, Modal Focus Order, Screen-Reader Toast Live Regions, SafeStorage Wrapper, Path-Agnostic Asset Fallbacks
...
- **Offline & Cache**: Service Worker + Cache Storage API with Resilient Promise.allSettled Offline Caching
...
tests/
	test_search.py    — ชุดทดสอบ Core Search Engine (14 Categories, 46 Assertions)
	test_full_system.py — ชุดตรวจสอบและตรวจสอบความปลอดภัยทั้งระบบ (Comprehensive Audit Suite: 45 Checks)
	test_runner.html  — หน้าทดสอบบนเบราว์เซอร์พร้อม UI แสดงผลและจับเวลา Latency (14 Categories, 46 Tests)
